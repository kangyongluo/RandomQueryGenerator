...
sub newFromDSN {
	...
	} elsif ($dsn =~ m/^dbi:odbc:/i) {
		require GenTest::Executor::TrafODBC;
		return GenTest::Executor::TrafODBC->new(dsn => $dsn, channel => $channel);
...
sub type {
	...
	if (ref($self) eq "GenTest::Executor::JavaDB") {
		...
		return DB_MYSQL;
	} elsif (ref($self) eq "GenTest::Executor::TrafODBC") {
		return DB_TRAFODBC;
...
my @dbid = ("Unknown","Dummy", "MySQL", "TrafODBC", "Postgres","JavaDB","Drizzle");
