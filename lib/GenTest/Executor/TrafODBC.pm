...
package GenTest::Executor::TrafODBC;
...
sub init {
	...
	my @tmp = split('dsn=', $executor->dsn());
	my @tmp1 = split(':', $tmp[1]); 
	my $dsn_odbc = $tmp1[0];

	@tmp = split('user=', $executor->dsn());
	@tmp1 = split(':', $tmp[1]); 
	my $user = $tmp1[0];

	@tmp = split('pwd=', $executor->dsn());
	@tmp1 = split(':', $tmp[1]); 
	my $pwd = $tmp1[0]; 

	@tmp = split('schema=', $executor->dsn());
	@tmp1 = split(':', $tmp[1]); 
	my $schema = $tmp1[0]; 
	
	if(not defined $dsn_odbc){
		$dsn_odbc = "traf";
	}
	if(not defined $user){
		$user = "trafodion";
	...
	say("odbc source info:");
	say("dsn:$dsn_odbc");
	say("user=$user");
	say("pwd=$pwd");
	say("schema=$schema");

	my $dbh = DBI->connect("DBI:ODBC:".$dsn_odbc, $user, $pwd, {
		PrintError => 1,
		RaiseError => 0,
		AutoCommit => 1,
	...
	$executor->defaultSchema($executor->currentSchema($schema));
...
sub execute {
	...
	if (defined $err) {			# Error on EXECUTE
		...
		if (
			...
			$dbh = DBI->connect("DBI:ODBC:traf", 'trafodion', 'traf123', {
				PrintError => 0,
				RaiseError => 0,
				AutoCommit => 1,
				mysql_multi_statements => 1
...
sub currentSchema {
	...
	return $executor->dbh()->selectrow_array("VALUES(CURRENT_SCHEMA)");
}
