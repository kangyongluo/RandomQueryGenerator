...
sub init {
    ...
    $dsn_part2 =~ s/([=;])/uc sprintf("=",ord($1))/eg;

    say("part1:$dsn_part1");
    say("part2:$dsn_part2");

    my @tmp = split('user=', $dsn_part2);
    my @tmp1 = split(':', $tmp[1]);
    my $user = $tmp1[0];

    @tmp = split('pwd=', $dsn_part2);
    @tmp1 = split(':', $tmp[1]);
    my $pwd = $tmp1[0];

    @tmp = split('schema=', $dsn_part2);
    @tmp1 = split(':', $tmp[1]);
    my $schema = $tmp1[0];
    ...
    say("jdbc source info:");
    say("user=$user");
    say("pwd=$pwd");
    say("schema=$schema");

    my $dbh = DBI->connect($dsn_part1."url=".$dsn_part2, $user, $pwd,
                           {
                               PrintError => 0,
                               RaiseError => 0,
                               AutoCommit => 1
    ...
    $self->defaultSchema($self->currentSchema($schema));
