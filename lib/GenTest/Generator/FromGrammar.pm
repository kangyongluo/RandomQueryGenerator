...
use constant GENERATOR_GRAMMAR_SQLTYPE => 13;
...
sub new {
	...
	my $generator = $class->SUPER::new({
		...
		'sqltype'	        => GENERATOR_GRAMMAR_SQLTYPE
	}, @_);
...
#		say("Loading grammar file '".$generator->grammarFile()."' ...");
    ...
    if(not defined $generator->sqltype()){
	$generator->[GENERATOR_GRAMMAR_SQLTYPE] = "mysql";
    }
...
sub sqltype {
	return $_[0]->[GENERATOR_GRAMMAR_SQLTYPE];
}
			...
			splice (@sentence, $pos, 1 , map {
				...
				if (exists $grammar_rules->{$_}) {
					...
					if ($rule_counters{$_} > GENERATOR_MAX_OCCURRENCES) {
						if($mask_level > 1){
							if($rule_counters{$_} > $mask_level){
								$last_value = $_;
								$is_longer = 1;
								if($mask_level > GENERATOR_MAX_OCCURRENCES){
									say("Rule $_ occured more than ".$mask_level." times. So the end.");
								}
								else{
									say("Rule $_ occured more than ".GENERATOR_MAX_OCCURRENCES." times. So the end.");
								}
								last;
							}	
							$grammar_rules->{$_};	
