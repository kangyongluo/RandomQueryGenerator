...
if ($process_type == PROCESS_TYPE_PARENT) {
	...
	say("Summary: ".($total_fail == 0 ? "test pass." : "test fail!")."  pid $periodic_pid thread $id queries=".($total_pass + $total_fail)."  pass=$total_pass  fail=$total_fail");
	if ($max_result > 0) {
